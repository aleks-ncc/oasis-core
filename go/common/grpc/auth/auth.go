...
func (a authServerStream) RecvMsg(m interface{}) error {
	if err := a.ss.RecvMsg(m); err != nil {
		return err
	}
	// Authenticate request.
	return a.authFunc(a.ss.Context(), a.fullMethod, m)
}

func (a authServerStream) SetHeader(m metadata.MD) error {
	return a.ss.SetHeader(m)
}

func (a authServerStream) SendHeader(m metadata.MD) error {
	return a.ss.SendHeader(m)
}

func (a authServerStream) SetTrailer(m metadata.MD) {
	a.ss.SetTrailer(m)
}
